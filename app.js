...
const { Telegraf, Stage, Scenes, session } = require("telegraf");
...
const stage = new Scenes.Stage([addSumScene, reduceSumScene, costScene]);
...
bot.command("budget", (ctx) =>
  ctx.reply("Чего вы хотите?", keyboardAddReduce.inline())
);
...
bot.action(["Add", "Reduce", "Cost"], async (ctx) => {
  ...
  if (data === "Add") {
    await ctx.editMessageText(
      "Загрузка данных...",
      keyboardAddReduce.removeKeyboard()
    );
